...
function Dungeon:update(dt)
    ...
    if not self.shifting then
        ...
        -- update pot movement too
        if self.player.stateMachine.currentState == "carry-walk" then
            self.player.stateMachine.current.object.x = self.player.x + self.player.width / 2 - self.player.stateMachine.current.object.width / 2
            self.player.stateMachine.current.object.y = self.player.y - self.player.height + 16
        end
    end
end
