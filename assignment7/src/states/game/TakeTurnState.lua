            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local initial = {health = self.playerPokemon.HP, attack = self.playerPokemon.attack, defense = self.playerPokemon.defense, speed = self.playerPokemon.speed}
                        local increase = self.playerPokemon:levelUp()
                        local final = {health = self.playerPokemon.HP, attack = self.playerPokemon.attack, defense = self.playerPokemon.defense, speed = self.playerPokemon.speed}

                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        function()
                            gStateStack:push(LevelUpState({initial = initial, increase = increase, final = final}))
                        end))
                    else
                        self:fadeOutWhite()
                    end
