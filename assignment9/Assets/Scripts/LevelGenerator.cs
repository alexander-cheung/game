...
public class LevelGenerator : MonoBehaviour {
	...
	public int floorTilesToRemove = 4;
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				// create floor
				// 10 percent chance to not have a floor; max 4 places; holes only where player can walk; no hole where char spawns or where coin is
				if (Random.value <= 0.08 && floorTilesToRemove > 0 && !mapData[z, x] && characterPlaced && (mazeX != x && mazeY != z)) {
					floorTilesToRemove--;
				} else {
