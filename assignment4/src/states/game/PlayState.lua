...
function PlayState:render()
    ...
    if self.player.key then
        love.graphics.draw(gTextures['keys_and_locks'], gFrames['keys_and_locks'][self.player.key], VIRTUAL_WIDTH - 16, 0)
    end
            ...
            if not groundFound then
                if self.tileMap.tiles[y][x].id == TILE_ID_GROUND then
                    ...
                    if math.random(20) == 1 then
                        ...
                        table.insert(self.map.entities, snail)
                    end
                end
            end
        end
