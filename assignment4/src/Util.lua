...
function GenerateTileSets(quads, setsX, setsY, sizeX, sizeY)
    ...
    for tilesetY = 1, setsY do
        for tilesetX = 1, setsX do
            ...
            -- orders tiles from quad generated (all tiles from left to right, up to down)
            -- into tileset order (every tile in tileset left right up down, then next tileset)
...
-- turns regular quads into flag quads only
function FlagsQuads(quads)
    local flags = {}
    -- 8th, 9th, and 10th columns are flags
    for n, quad in pairs(quads) do
        if (n - 7) % 9 == 0 or (n - 8) % 9 == 0 or n % 9 == 0 then
            table.insert(flags, quad)
        end
    end
    return flags
end
